...
(function($) {
    ...
    var showSortModal = function(that) {
        ...
        if (!$(_id).hasClass("modal")) {
            ...
            sModal += '                     <button type="button" class="btn btn-default" data-dismiss="modal" style="text-align:center">' + that.options.formatCancel() + '</button>';
            sModal += '                     <button type="button" class="btn btn-primary" style="width:54px;height:30px;">' + that.options.formatSort() + '</button>';
            sModal += '                 </div>';
            sModal += '             </div>';
            sModal += '         </div>';
            sModal += '     </div>';
        ...
        /*支持拖拽依赖：http://code.jquery.com/ui/1.11.4/themes/smoothness/jquery-ui.css和http://code.jquery.com/ui/1.11.4/themes/smoothness/jquery-ui.css*/
    ...
    BootstrapTable.prototype.initToolbar = function() {
        ...
        if (this.options.showMultiSort) {
            ...
            if (!$multiSortBtn.length) {
                $multiSortBtn = '  <button style="width:52px;height:34px;" class="multi-sort btn btn-default' + (this.options.iconSize === undefined ? '' : ' btn-' + this.options.iconSize) + '" type="button" data-toggle="modal" data-target="' + sortModalId + '" title="' + this.options.formatMultipleSort() + '" >';
    ...
    BootstrapTable.prototype.addLevel = function(index, sortPriority) {
        ...
        $.each(this.columns, function(i, column) {
            ...
            //start fixed render muti-sort sortName not work add by 傅为地
            if(column.sortName){
            	$multiSortName.append('<option value="' + column.sortName + '">' + column.title + '</option>');
            }
            else{
            	$multiSortName.append('<option value="' + column.field + '">' + column.title + '</option>');
            }
            //end fixed render muti-sort sortName not work add by 傅为地
            /*$multiSortName.append('<option value="' + column.field + '">' + column.title + '</option>');*/
        });
        ...
        for (var i = 0; i < headers.length; i++) {
        	if(that.options.sortPriority!=null){
        		for (var c = 0; c < that.options.sortPriority.length; c++) {
        			...
        			//start fixed after muti-sort action the title do not show asc or desc icon add by 傅为地 20170419 
                    if ($(headers[i]).data('field') === that.options.sortPriority[c].sortName||$(headers[i]).data('sortName') === that.options.sortPriority[c].sortName) {
                        $(headers[i]).find('.sortable').removeClass('desc asc').addClass(that.options.sortPriority[c].sortOrder);
                    }
                    //end fixed after muti-sort action the title do not show asc or desc icon add by 傅为地 20170419 
                }
        	}
        }
    };
